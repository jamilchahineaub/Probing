...
def _delivery_summary(rows: Sequence[Mapping[str, Any]], config: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    metrics = {
        ...
        "median_probe_separation_count_completed": _quantile(complete, "probe_separation_count", 0.5, finite_only=True),
        "p95_probe_separation_count_completed": _quantile(complete, "probe_separation_count", 0.95, finite_only=True),
        "median_probe_recontact_count_completed": _quantile(complete, "probe_recontact_count", 0.5, finite_only=True),
        "median_probe_contact_fraction_completed": _quantile(complete, "probe_contact_fraction", 0.5, finite_only=True),
        ...
        "median_probe_correlation_completed": _quantile(complete, "probe_correlation", 0.5, finite_only=True),
        "passive_window_available_fraction": len(complete) / max(len(rows), 1),
        "passive_recontact_fraction_completed": float(np.mean([bool(row["passive_recontact"]) for row in complete])) if complete else 1.0,
    ...
    checks = {
        ...
        "passive_recontact": metrics["passive_recontact_fraction_completed"] <= float(gates["passive_recontact_trial_fraction"]),
    ...
    for trial_index, physical in enumerate(physical_rows):
        ...
        for regime_index, (noise_name, multiplier) in enumerate(regimes.items()):
            ...
            output.append({
                ...
                "damping_n_s_per_m": physical["damping_n_s_per_m"],
                "effective_mass_kg": physical["effective_mass_kg"],
                "terminal_phase": physical["terminal_phase"],
                "terminal_reason": physical["terminal_reason"],
                "probe_rms_tracking_error_n": physical["probe_rms_tracking_error_n"],
                "probe_normalized_rms_tracking_error": physical["probe_normalized_rms_tracking_error"],
                "probe_cross_correlation_lag_s": physical["probe_cross_correlation_lag_s"],
                "probe_delivery_bandwidth_hz": physical["probe_delivery_bandwidth_hz"],
                "probe_separation_count": physical["probe_separation_count"],
                "passive_residual_force_rms_n": physical["passive_residual_force_rms_n"],
                "future_peak_displacement_m": physical["future_peak_displacement_m"],
                "future_peak_velocity_m_per_s": physical["future_peak_velocity_m_per_s"],
                "future_late_hold_oscillation_rms_m": physical["future_late_hold_oscillation_rms_m"],
                "future_hold_settling_time_s": physical["future_hold_settling_time_s"],
