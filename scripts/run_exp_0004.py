#!/usr/bin/env python3
"""Run immutable EXP-0004 Stage 1 sequential-probing artifacts."""
...
from probeing.experiments import run_sequential_identification
from probeing.plotting import plot_sequential_identification
...
def _load_config(path: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(config, dict) or config.get("schema_version") != 1:
        raise ValueError("EXP-0004 requires a schema_version 1 YAML mapping")
    if config.get("experiment", {}).get("id") != "EXP-0004":
        raise ValueError("this entry point only runs EXP-0004")
    seeds = [int(seed) for seed in config["validation_seeds"]]
    forbidden = [
        int(seed)
        for seed in config["integrity_acceptance"]["forbidden_validation_seeds_before_final_run"]
    ]
    if seeds != forbidden:
        raise ValueError("final validation seed list must equal the predeclared untouched seed list")
    return config
...
def run(config_path: Path, runs_root: Path, results_root: Path) -> tuple[str, bool, str]:
    ...
    predecessor_ids = ("EXP-0001", "EXP-0002", "EXP-0003")
    before = {
        experiment_id: _experiment_fingerprint(REPOSITORY_ROOT, experiment_id)
        for experiment_id in predecessor_ids
    }
    expected = {
        experiment_id.upper().replace("_", "-"): str(reference["expected_artifact_fingerprint"])
    ...
    run_id = _new_run_id("EXP-0004", timestamp, seeds[0])
    ...
    try:
        ...
        result = run_sequential_identification(config, progress_callback=progress)
        paths = {
            "probe_trials_csv": run_directory / "probe_trials.csv.gz",
            "probe_trials_npz": run_directory / "probe_trials.npz",
            "selection_candidates_csv": run_directory / "selection_candidates.csv.gz",
            "selection_candidates_npz": run_directory / "selection_candidates.npz",
            "stage_aggregate": run_directory / "stage_aggregate.csv",
            "strategy_summary": run_directory / "strategy_summary.csv",
            "frequency_information": run_directory / "frequency_information.csv",
        ...
        _write_rows_gzip(paths["probe_trials_csv"], result.trial_rows)
        _write_rows_npz(paths["probe_trials_npz"], result.trial_rows)
        _write_rows_gzip(paths["selection_candidates_csv"], result.selection_rows)
        _write_rows_npz(paths["selection_candidates_npz"], result.selection_rows)
        _write_row_table(paths["stage_aggregate"], result.stage_aggregate)
        _write_row_table(paths["strategy_summary"], result.strategy_summary)
        _write_row_table(paths["frequency_information"], result.frequency_information)
        ...
        _write_json(
            paths["stage1_decision"],
            {
                ...
                "sequential_improvement_is_significant": result.summary[
                    "sequential_improvement_is_significant"
                ],
                "matched_fixed_comparator": result.summary["matched_fixed_comparator"],
            },
        )

        figures = plot_sequential_identification(
            trial_rows=result.trial_rows,
            selection_rows=result.selection_rows,
            stage_aggregate=result.stage_aggregate,
            strategy_summary=result.strategy_summary,
            frequency_information=result.frequency_information,
        ...
        for name in ("stage_aggregate", "strategy_summary", "frequency_information"):
        ...
        if after != before:
            raise RuntimeError("EXP-0001, EXP-0002, or EXP-0003 artifacts changed during EXP-0004")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        / "configs"
        / "experiments"
        / "exp_0004_sequential_active_identification.yaml",
    )
