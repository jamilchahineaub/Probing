...
- Stage 1/2 remained frozen at commit
  `95fcdbb5d08f2ffa46077aced7f5c882fb2627f8` and tag
  `stage2-complete-20260830`; all seven Stage 1 fingerprints matched and the
  pre-run Python suite passed `98/98` after adding Stage 3A tests.
- The vehicle uses explicit ENU/FLU frame conventions, scalar-first Hamilton
  quaternions, four individual quadratic-thrust/reaction-torque rotors,
  first-order 30 ms motor lag, speed saturation, and full inertia.
- A rigid massless probe is offset `[0.30, 0, -0.08] m` from the CoM. The
  unilateral penalty interface transmits both normal force and the exact
  `r_probe × F_contact` body moment to the vehicle.
- The unknown structure retains the frozen one-dimensional
  `m_eff*x_ddot + c*x_dot + k*x = F_contact` dynamics.
- A conventional position/velocity plus geometric attitude/rate controller
  attempts to realize the unchanged positive unilateral portion of the 0.5 N,
  0.5–5 Hz chirp. The command is exactly zero during the 0.5 s passive window.
  Contact-torque feedforward uses only measured force and known rigid geometry.
- The EXP-0006 policy bundle, features, normalization, regressors, thresholds,
  safety labels, and observation duration were applied without tuning.

### Validation

- Thirteen no-contact checks passed: analytical free fall, exact hover,
  meaningful-duration hover, translation, roll, yaw, rotor/mixer signs, motor
  lag, saturation, rotational sign, quaternion norm, and zero contact wrench.
- Nine contact checks passed: separation, normal direction, through-CoM zero
  torque, offset cross product and pitch signs above/below CoM, analytical
  stationary press equilibrium, non-growing mechanical energy, and analytical
  free ring-down.
- In the difficult convergence case, reducing step size from 1 ms to 0.5 ms
  changed RMS tracking error by `0.02%`, peak contact force by `0.02%`, and
  peak target displacement by `0.13%`.
- Independent MATLAB full equations reproduced the stiff representative force
  trace within `1.13e-11 N` maximum error. Simulink subsystem checks for hover,
  offset rotation, and coupled normal target/contact executed successfully.

### Reference run

- Run ID: `EXP-0008_20260830T123909.142216Z_s13101_f41ef89f`.
- 107 physical configurations: seven representatives, 27 structured target
  combinations, 60 untouched Monte Carlo targets on seeds 13101–13105, and 13
  one-factor vehicle/contact/actuation variations.
- 321 locked-policy evaluations across low, nominal, and high noise.
- [Manifest](runs/EXP-0008_20260830T123909.142216Z_s13101_f41ef89f/manifest.json)
- [Summary](runs/EXP-0008_20260830T123909.142216Z_s13101_f41ef89f/summary.json)
- [Post-run mechanism audit](runs/EXP-0008_20260830T123909.142216Z_s13101_f41ef89f/postrun_analysis.json)

### Probe delivery

- Median force RMS tracking error: `0.399 N`; p95 `0.527 N`.
- Median relative RMS error: `152.23%`; p95 `200.94%`.
- Median bounded cross-correlation delay: `224 ms`.
- Median declared faithful bandwidth: `2.00 Hz`; 48/107 trials (`44.86%`)
  had no 0.5–5 Hz bin within both ±3 dB gain and 45° phase.
- Median/maximum realized peak force: `2.52/4.99 N` despite a `0.5 N`
  command. Re-contact occurred in all 107 trials. The 1.25 N prospective force
  limit was exceeded in 101 trials; one additional acceleration event brought
  the probe-safety-event count to 102.
- Maximum attitude disturbance was `2.89°`; no motor saturated in any trial;
  minimum observed actuator reserve was `54.07%`. The dominant failure is not
  thrust authority or attitude loss—it is hybrid contact delivery and rebound.

### Frozen decision result

The 94-target population excludes the 13 repeated one-factor variants from
the primary target-distribution metric. Those variants are reported separately.

| Population | Noise | Accuracy | False-safe | One-sided 95% upper |
|---|---|---:|---:|---:|
| Monte Carlo, 60 | nominal | 83.33% | 5/32 = 15.63% | 30.08% |
| Monte Carlo, 60 | high | 83.33% | 6/32 = 18.75% | 33.69% |
| Full target population, 94 | nominal | 82.98% | 6/51 = 11.76% | 21.91% |
| Full target population, 94 | high | 81.91% | 8/51 = 15.69% | 26.53% |
| Boundary, 20 | nominal | 50.00% | 8/15 = 53.33% | 75.63% |
| Boundary, 20 | high | 40.00% | 9/15 = 60.00% | 80.91% |

Across all 107 configurations, nominal/high accuracy was `72.90%/72.90%`
and false-safe was `29.69%/31.25%`; this aggregate is intentionally dominated
by the 13 one-factor variants of one target, all of which became NON-SAFE under
the coupled sustained maneuver.

### Mechanism

- Coupling changed 33/107 binary physical labels relative to the reduced 2 N
  target-only maneuver, including 20/94 of the target-population cases.
- Median reduced/coupled settling times were `0.0/2.822 s`.
- All 19 nominal false-safe configurations violated only the settling-time
  criterion. None exceeded the SAFE peak-displacement, velocity, oscillation,
  or contact-travel thresholds. Every one of those 19 had a ground-truth label
  changed by coupling.
- Passive ring-down remains observable and the frozen persistence veto fired
  on 21.50% of nominal trials. It cannot predict excitation introduced later
  by the sustained-contact vehicle/controller loop.

### Decision

EXP-0008 completes a scientifically meaningful Stage 3A baseline but the
frozen safety policy **does not transfer safely**. The reduced-order signal is
still visible—nominal target-population accuracy remains 82.98%—but false-safe
and probe-force violations are unacceptable. This is not a Python numerical
artifact and is not caused by actuator saturation.

Decision: **CONTINUE_STAGE_3A**. Do not tune EXP-0008 or begin Stage 3B. The
next experiment should prevalidate a hybrid normal-force/contact-state
controller that explicitly unloads before rebound and achieves stable bounded
sustained-contact tracking. It should then replay the frozen policy first,
separating probe-delivery failure from future-maneuver control failure.
